...
export default function ComportementPage() {
  const { data, q, narrow, serverError, failedEndpoints, loading } = useAnalyticsData([
    ...
    { key: "abandonedCarts", path: "/api/admin/abandoned-carts", kind: "raw",
      normalize: (j: any) => (j?.carts && Array.isArray(j.carts)) ? { value: j.carts, ok: true } : Array.isArray(j) ? { value: j, ok: true } : { value: [], ok: false } },
  ]);
