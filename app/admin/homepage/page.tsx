...
const S = {
  ...
  btnRm:   { position: "absolute" as const, top: 6, right: 6, width: 22, height: 22, borderRadius: 99, background: "rgba(220,38,38,0.9)", color: "#fff", border: "none", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900 },
};
...
export default function AdminHomePage() {
  ...
  // Produits sélectionnés avec leurs données
    ...
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div>
            <label style={S.label}>Choisir un titre prédéfini</label>
            <select
        ...
        <div style={{ marginTop: 14, padding: "12px 18px", borderRadius: 10, background: "#f5f0e8", border: "1px solid rgba(196,154,74,0.2)" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(26,20,16,0.4)", marginBottom: 4 }}>APERÇU HOMEPAGE</div>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          {selectedIds.length > 0 && (
            <span style={{ fontSize: 12, color: "rgba(26,20,16,0.4)" }}>Glisse pour réordonner ↕</span>
          )}
        </div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {selectedProducts.map((p: any, idx) => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 12, background: "#faf8f4", border: "1px solid rgba(26,20,16,0.08)" }}>
                ...
                {/* Image */}
                <div style={{ width: 44, height: 44, borderRadius: 8, overflow: "hidden", background: "#ede8df", flexShrink: 0, position: "relative" }}>
                  {p.image_url
                    ? <Image src={p.image_url} alt={p.name} fill sizes="44px" style={{ objectFit: "cover" }} />
                    : <div style={{ inset: 0, position: "absolute", display: "grid", placeItems: "center", fontSize: 10, color: "rgba(26,20,16,0.2)", fontWeight: 900 }}>M!LK</div>
                  }
                </div>
                ...
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  <button onClick={() => moveUp(idx)} disabled={idx === 0}
                    style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid rgba(26,20,16,0.12)", background: "transparent", cursor: idx === 0 ? "default" : "pointer", opacity: idx === 0 ? 0.3 : 1, fontSize: 12 }}>
                    ↑
                  </button>
                  <button onClick={() => moveDown(idx)} disabled={idx === selectedIds.length - 1}
                    style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid rgba(26,20,16,0.12)", background: "transparent", cursor: idx === selectedIds.length - 1 ? "default" : "pointer", opacity: idx === selectedIds.length - 1 ? 0.3 : 1, fontSize: 12 }}>
                    ↓
                  </button>
                  <button onClick={() => remove(p.id)}
                    style={{ width: 28, height: 28, borderRadius: 6, border: "1px solid rgba(220,38,38,0.2)", background: "rgba(220,38,38,0.05)", cursor: "pointer", color: "#dc2626", fontSize: 14, fontWeight: 900 }}>
                    ×
                  </button>
                </div>
              </div>
      ...
      {/* ── 3. CATALOGUE ── */}
      <div style={S.card}>
        <h2 style={S.cardH}>📦 Catalogue produits</h2>
          ...
          {filtered.map((p: any) => {
            ...
            return (
              <button
                ...
                style={{
                  borderRadius: 12,
                  border: isSelected ? "2px solid #c49a4a" : "1px solid rgba(26,20,16,0.1)",
              ...
              >
                ...
                {isSelected && (
                  <div style={{ position: "absolute", top: 6, right: 6, zIndex: 2, width: 22, height: 22, borderRadius: 99, background: "#c49a4a", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ color: "#1a1410", fontSize: 12, fontWeight: 900 }}>✓</span>
                  </div>
                )}
                {/* Rupture */}
                {p.stock <= 0 && (
                ...
                {/* Image */}
                <div style={{ width: "100%", aspectRatio: "1/1", background: "#ede8df", position: "relative" }}>
                  {p.image_url
                    ? <Image src={p.image_url} alt={p.name} fill sizes="160px" style={{ objectFit: "cover" }} />
                    : <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", fontSize: 12, fontWeight: 900, color: "rgba(26,20,16,0.2)" }}>M!LK</div>
                  }
                </div>
                {/* Texte */}
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, alignItems: "center" }}>
        <span style={{ fontSize: 13, color: "rgba(26,20,16,0.4)" }}>
          {selectedIds.length} produit{selectedIds.length !== 1 ? "s" : ""} · titre : "{sectionTitle}"
        </span>
        <button onClick={save} disabled={saving} style={{ ...S.btnSave, opacity: saving ? 0.6 : 1 }}>
          {saving ? "Sauvegarde…" : "Sauvegarder la homepage →"}
        </button>
      </div>
    </div>
  );
