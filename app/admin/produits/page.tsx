...
// ✅ Détection motif depuis le slug
function getMotif(slug: string): { label: string; icon: string; bg: string; color: string } | null {
  if (slug.includes("eclair"))  return { label: "Éclair",  icon: "⚡", bg: "rgba(26,20,16,0.08)",  color: "#1a1410" };
  if (slug.includes("smileys")) return { label: "Smileys", icon: "☺", bg: "rgba(212,165,116,0.15)", color: "#92580a" };
  if (slug.includes("damier"))  return { label: "Damier",  icon: "✦", bg: "rgba(80,80,80,0.1)",     color: "#444444" };
  if (slug.includes("uni"))     return { label: "Uni",     icon: "▤", bg: "rgba(139,94,60,0.12)",   color: "#6b3e1e" };
  return null;
}
...
export default function AdminProduitsListe() {
  ...
  const filtered = products.filter(p => {
    ...
    const matchMotif  = !motifFilter  || (p.slug ?? "").includes(motifFilter);
  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 1120 }}>
            ...
            {exporting ? (
              ...
              <>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
                  <path d="M12 3v13M7 12l5 5 5-5M4 20h16" stroke="#c49a4a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                Export stock
              </>
            )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={s.label} style={{ background: "#fff", borderRadius: 14, border: "1px solid rgba(0,0,0,0.07)", padding: "14px 16px", textAlign: "center" }}>
            <div style={{ fontSize: typeof s.value === "string" ? 17 : 24, fontWeight: 950, color: s.color, lineHeight: 1 }}>{s.value}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10, marginBottom: 28 }}>
        {statsByCat.map(s => (
          <button key={s.label} onClick={() => setCatFilter(prev => prev === Object.keys(CATEGORY_LABEL).find(k => CATEGORY_LABEL[k] === s.label)! ? "" : Object.keys(CATEGORY_LABEL).find(k => CATEGORY_LABEL[k] === s.label)!)}
            style={{ padding: "14px 16px", borderRadius: 14, border: `1px solid ${catFilter === Object.keys(CATEGORY_LABEL).find(k => CATEGORY_LABEL[k] === s.label) ? "rgba(196,154,74,0.4)" : "rgba(0,0,0,0.07)"}`, background: catFilter === Object.keys(CATEGORY_LABEL).find(k => CATEGORY_LABEL[k] === s.label) ? "rgba(196,154,74,0.08)" : "#fff", cursor: "pointer", textAlign: "left" }}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {MOTIFS.map(m => (
            <button key={m.value} onClick={() => setMotifFilter(m.value)}
              style={{ padding: "9px 14px", borderRadius: 99, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, background: motifFilter === m.value ? "#1a1410" : "rgba(26,20,16,0.07)", color: motifFilter === m.value ? "#c49a4a" : "rgba(26,20,16,0.6)", transition: "all 0.15s" }}>
              {m.label}
            </button>
          ))}
        </div>
                  ...
                  >
                    ...
                    <td style={{ padding: "13px 14px" }}>
                      <button onClick={() => togglePublish(p.id, p.published !== false)}
                        title={p.published !== false ? "Dépublier" : "Publier"}
                    ...
                    <td style={{ padding: "13px 14px", maxWidth: 220 }}>
                      ...
                      <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", marginTop: 3 }}>
                        ...
                        {p.supplier_ref && (
                          <span style={{ fontSize: 10, fontWeight: 800, color: "#c49a4a", background: "rgba(196,154,74,0.1)", padding: "2px 6px", borderRadius: 99 }}>
                            {p.supplier_ref}
                          </span>
                        )}
                      </div>
                    ...
                    {/* ✅ Motif — badge sans couleurs */}
                    <td style={{ padding: "13px 14px" }}>
                      {motif ? (
                        <span style={{ padding: "5px 12px", borderRadius: 99, background: motif.bg, fontSize: 13, fontWeight: 800, color: motif.color, whiteSpace: "nowrap", display: "inline-flex", alignItems: "center", gap: 5 }}>
                    ...
                    <td style={{ padding: "13px 14px" }}>
                      <span style={{
                        padding: "4px 12px", borderRadius: 99, fontSize: 13, fontWeight: 800, whiteSpace: "nowrap",
                        background: !p.stock ? "#f3f4f6" : p.stock <= 5 ? "rgba(196,154,74,0.15)" : "#dcfce7",
                        color:      !p.stock ? "#6b7280" : p.stock <= 5 ? "#c49a4a"              : "#166534",
                      }}>
                        {p.stock ?? 0} u.
                      </span>
                    </td>
