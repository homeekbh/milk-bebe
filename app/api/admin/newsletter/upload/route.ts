...
/**
 * Upload d'une image de newsletter vers Supabase Storage (bucket public
 * product-images, dossier newsletter/). Service role. Vérif magic-bytes pour
 * éviter le spoofing de Content-Type. Retourne { url } (URL publique).
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Nom de fichier sécurisé : timestamp + slug du nom d'origine + ext du MIME
    const extMap: Record<string, string> = { "image/jpeg": "jpg", "image/png": "png", "image/webp": "webp" };
    const ext  = extMap[file.type];
    const base = (file.name || "image").replace(/\.[^.]+$/, "")
      .toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "")
      .replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 40) || "image";
    const path = `newsletter/${Date.now()}_${base}.${ext}`;
