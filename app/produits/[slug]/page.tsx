    ...
    <div style={{ background: BG, minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: 1800, margin: "0 auto", padding: "0 4vw 80px" }}>
        <div className="bottom-grid" style={{ marginBottom: 24 }}>
          ...
          {/* ── Sélecteur de motifs (même catégorie) ── */}
          {related.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2, textTransform: "uppercase", color: AMBER, marginBottom: 10 }}>
                Autres motifs disponibles
              </div>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                {/* Pastille du produit courant (actif) */}
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 56, height: 56, borderRadius: 14, overflow: "hidden", border: `3px solid ${AMBER}`, background: TAUPE, flexShrink: 0, position: "relative" }}>
                    {product.image_url
                      ? <Image src={product.image_url} alt={product.name} fill sizes="56px" style={{ objectFit: "cover" }} />
                      : <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", fontSize: 9, fontWeight: 900, color: "rgba(26,20,16,0.3)" }}>M!LK</div>}
                  </div>
                  <span style={{ fontSize: 11, fontWeight: 800, color: DARK, textAlign: "center", maxWidth: 64, lineHeight: 1.3 }}>
                    {(() => {
                      const parts = product.name.split("—");
                      return parts.length > 1 ? parts[parts.length - 1].trim() : product.name;
                    })()}
                  </span>
                </div>

                {/* Pastilles des autres motifs */}
                {related.map((p: any) => {
                  const motifName = (() => {
                    const parts = (p.name ?? "").split("—");
                    return parts.length > 1 ? parts[parts.length - 1].trim() : p.name;
                  })();
                  return (
                    <Link key={p.id} href={`/produits/${p.slug}`}
                      style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, textDecoration: "none" }}>
                      <div style={{ width: 56, height: 56, borderRadius: 14, overflow: "hidden", border: "2px solid rgba(26,20,16,0.12)", background: TAUPE, flexShrink: 0, position: "relative", transition: "border-color 0.15s, transform 0.15s" }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = AMBER; (e.currentTarget as HTMLElement).style.transform = "scale(1.08)"; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(26,20,16,0.12)"; (e.currentTarget as HTMLElement).style.transform = ""; }}>
                        {p.image_url
                          ? <Image src={p.image_url} alt={p.name} fill sizes="56px" style={{ objectFit: "cover" }} />
                          : <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", fontSize: 9, fontWeight: 900, color: "rgba(26,20,16,0.3)" }}>M!LK</div>}
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(26,20,16,0.55)", textAlign: "center", maxWidth: 64, lineHeight: 1.3 }}>{motifName}</span>
                    </Link>
                  );
                })}
              </div>
