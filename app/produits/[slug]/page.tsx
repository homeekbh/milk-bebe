...
function DiagonalBadge({ label, out }: { label?: string; out: boolean }) {
  ...
  return (
    <div style={{ position: "absolute", top: 0, right: 0, width: 120, height: 120, overflow: "hidden", zIndex: 30, pointerEvents: "none" }}>
      <div style={{ position: "absolute", top: 28, right: -34, background: "#c49a4a", color: "#1a1410", fontSize: 11, fontWeight: 900, letterSpacing: 0.5, padding: "9px 48px", transform: "rotate(45deg)", textTransform: "uppercase", whiteSpace: "nowrap", boxShadow: "0 3px 14px rgba(0,0,0,0.4)" }}>
        {text}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#221c16", borderRadius: 24, padding: 32, maxWidth: 480, width: "100%", border: "1px solid rgba(242,237,230,0.1)" }}
      >
          ...
          <thead>
            <tr>
              {["Taille", "Poids", "Hauteur", "Âge"].map(h => (
                <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 11, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", color: "rgba(242,237,230,0.35)", borderBottom: "1px solid rgba(242,237,230,0.1)" }}>
                  {h}
                </th>
              ))}
            </tr>
        ...
        <p style={{ margin: "16px 0 0", fontSize: 13, color: "rgba(242,237,230,0.4)", lineHeight: 1.7 }}>
          En cas de doute, prenez la taille supérieure.
        </p>
        <button
          onClick={onClose}
          style={{ marginTop: 20, width: "100%", padding: "14px", borderRadius: 12, background: "#f2ede6", color: "#1a1410", fontWeight: 900, fontSize: 15, border: "none", cursor: "pointer" }}
        >
          Fermer
        </button>
      </div>
...
function FaqItem({ q, r }: { q: string; r: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(242,237,230,0.08)" }}>
      ...
      {open && (
        <div style={{ paddingBottom: 18, fontSize: 14, lineHeight: 1.8, color: "rgba(242,237,230,0.55)" }}>{r}</div>
      )}
    </div>
  );
}
      ...
      if (found && !found.error) {
        ...
        setRelated(
          (Array.isArray(all) ? all : [])
            .filter((p: any) => p.id !== found.id && p.category_slug === found.category_slug && p.published !== false)
            .slice(0, 3)
        );
  ...
  return (
    <div style={{ background: "#f5f0e8", minHeight: "100vh" }}>
      <style>{`
        ...
        .pl-sticky  { position: sticky; top: 100px; }
        .pl-compo   { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-top: 80px; }
        .pl-pad     { padding: 24px 32px 100px; }
        .pl-zhint   { display: block; }
        ...
        @media (max-width: 768px) {
          ...
          .pl-zhint  { display: none !important; }
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }} className="pl-pad">
        <div className="pl-layout">
          ...
          {/* ── Galerie ── */}
          <div className="pl-sticky" style={{ display: "grid", gap: 12 }}>

            {/* ✅ Wrapper relatif pour le badge — EN DEHORS du overflow:hidden */}
            <div style={{ position: "relative" }}>
              ...
              >
                {images[activeImg] ? (
                  ...
                  <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", fontWeight: 950, fontSize: 48, color: "#c8bfb2" }}>
                    M!LK
                  </div>
                )}
                ...
                {lowStock && (
                  <div style={{ position: "absolute", top: 14, left: 14, zIndex: 5 }}>
                    <span style={{ padding: "6px 12px", borderRadius: 99, background: "rgba(180,80,60,0.85)", color: "#fff", fontSize: 11, fontWeight: 800 }}>
                      Plus que {product.stock} !
                    </span>
                  </div>
                )}

                <div className="pl-zhint" style={{ position: "absolute", bottom: 14, right: 14, padding: "5px 10px", borderRadius: 99, background: "rgba(26,20,16,0.45)", color: "#fff", fontSize: 11, fontWeight: 600 }}>
          ...
          <div style={{ display: "grid", gap: 24 }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
              <span style={{ fontSize: 32, fontWeight: 950, letterSpacing: -1, color: "#1a1410" }}>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 72 }}>
            <h2 style={{ margin: "0 0 24px", fontSize: "clamp(18px, 3vw, 28px)", fontWeight: 950, letterSpacing: -1, color: "#1a1410" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20 }}>
              {related.map((p: any) => (
                <Link
                  ...
                  style={{ textDecoration: "none", color: "inherit", display: "block", borderRadius: 20, overflow: "hidden", background: "#fff", border: "1px solid rgba(26,20,16,0.07)" }}
                >
      ...
      {/* CTA Sticky mobile */}
      <div className="pl-mcta" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 50, padding: "12px 16px", background: "rgba(245,240,232,0.97)", backdropFilter: "blur(8px)", borderTop: "1px solid rgba(26,20,16,0.1)", boxShadow: "0 -4px 20px rgba(0,0,0,0.08)" }}>
