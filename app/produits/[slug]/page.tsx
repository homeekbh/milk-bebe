...
function DiagonalBadge({ label, out }: { label?: string; out: boolean }) {
  if (out) {
    return (
      <div style={{ position: "absolute", top: 0, right: 0, width: 110, height: 110, overflow: "hidden", zIndex: 30, pointerEvents: "none" }}>
        <div style={{ position: "absolute", top: 26, right: -30, background: "#6b7280", color: "#fff", fontSize: 11, fontWeight: 900, letterSpacing: 1, padding: "8px 44px", transform: "rotate(45deg)", textTransform: "uppercase", whiteSpace: "nowrap" }}>Épuisé</div>
      </div>
    );
  }
  ...
  const text = label && cfg[label] ? cfg[label] : null;
  if (!text) return null;
  return (
...
// ✅ SVG icons sans emoji — couleurs site uniquement
function IconThermometer() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 2v10m0 0a4 4 0 1 0 0 8 4 4 0 0 0 0-8z" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/><path d="M12 6h2M12 9h1" stroke="#c49a4a" strokeWidth="1.5" strokeLinecap="round"/></svg>;
}
function IconBan() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9" stroke="#c49a4a" strokeWidth="1.8"/><path d="M6 6l12 12" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/></svg>;
}
function IconFlat() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 12h16M4 8h8M4 16h8" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/></svg>;
}
function IconHeat() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M8 6c0 2 2 2 2 4s-2 2-2 4" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/><path d="M12 4c0 2 2 2 2 4s-2 2-2 4" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/><path d="M16 6c0 2 2 2 2 4s-2 2-2 4" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/><path d="M5 19h14" stroke="#f2ede6" strokeWidth="1.8" strokeLinecap="round"/></svg>;
}
function IconLeaf() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 22C12 22 4 16 4 9a8 8 0 0 1 16 0c0 7-8 13-8 13z" stroke="#c49a4a" strokeWidth="1.8"/><path d="M12 22V9" stroke="#c49a4a" strokeWidth="1.8"/></svg>;
}
function IconTruck() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M1 3h13v13H1z" stroke="#c49a4a" strokeWidth="1.8" strokeLinejoin="round"/><path d="M14 8h4l3 3v5h-7V8z" stroke="#c49a4a" strokeWidth="1.8" strokeLinejoin="round"/><circle cx="5.5" cy="18.5" r="2.5" stroke="#c49a4a" strokeWidth="1.8"/><circle cx="18.5" cy="18.5" r="2.5" stroke="#c49a4a" strokeWidth="1.8"/></svg>;
}
function IconReturn() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M9 14H4V9" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/><path d="M4 14a9 9 0 1 0 1.5-5" stroke="#c49a4a" strokeWidth="1.8" strokeLinecap="round"/></svg>;
}
function IconLock() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><rect x="3" y="11" width="18" height="11" rx="2" stroke="#c49a4a" strokeWidth="1.8"/><path d="M7 11V7a5 5 0 0 1 10 0v4" stroke="#c49a4a" strokeWidth="1.8"/></svg>;
}

// ✅ FAQ compact — tout sur une ligne, espace réduit
function FaqItem({ q, r }: { q: string; r: string }) {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ borderTop: "1px solid rgba(242,237,230,0.07)" }}>
      <button onClick={() => setOpen(v => !v)}
        style={{ width: "100%", padding: "14px 0", background: "none", border: "none", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, textAlign: "left" }}>
        <span style={{ fontWeight: 800, fontSize: "clamp(13px, 1.5vw, 16px)", color: "#f2ede6", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{q}</span>
        <span style={{ fontSize: 20, color: "#c49a4a", flexShrink: 0, transition: "transform 0.2s", transform: open ? "rotate(45deg)" : "none" }}>+</span>
      </button>
      {open && (
        <div style={{ padding: "0 0 14px", fontSize: "clamp(13px, 1.3vw, 15px)", lineHeight: 1.7, color: "rgba(242,237,230,0.55)" }}>
          {r}
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ background: "#f5f0e8", minHeight: "100vh" }}>
      <style>{`
        ...
        .pl-pad    { padding: 20px 4vw 80px; }
        .pl-compo  { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 48px; }
        ...
        @media (max-width: 768px) {
          ...
          .pl-pad    { padding: 16px 4vw 120px !important; }
      ...
      <div className="pl-pad">
        <div className="pl-layout">
          ...
          <div>
            <div style={{ position: "relative" }}>
              ...
              <div className="photo-grid">
                {images.length === 0 ? (
                  <div style={{ gridColumn: "1/-1", aspectRatio: "1", background: "#ede8df", borderRadius: 16, display: "grid", placeItems: "center", fontSize: 30, fontWeight: 950, color: "#c8bfb2" }}>M!LK</div>
                ) : images.map((img, i) => (
                  <div key={i}
                    ...
                    <Image src={img} alt={`${product.name} ${i+1}`} fill sizes="(max-width:768px) 100vw, 50vw" style={{ objectFit: "cover", transition: "transform 0.15s ease" }} />
            ...
            {/* ✅ Guide des tailles TOUJOURS VISIBLE — inline sous les photos */}
            <div style={{ marginTop: 16, background: "#fff", borderRadius: 14, border: "1px solid rgba(26,20,16,0.08)", overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", background: "#1a1410", display: "flex", alignItems: "center", gap: 8 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M3 6h18M3 12h10M3 18h6" stroke="#c49a4a" strokeWidth="2" strokeLinecap="round"/></svg>
                <span style={{ fontSize: 12, fontWeight: 900, letterSpacing: 2, textTransform: "uppercase", color: "#c49a4a" }}>Guide des tailles</span>
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#f9f6f2" }}>
                    {["Taille", "Poids", "Hauteur", "Âge"].map(h => (
                      <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 800, letterSpacing: 0.5, textTransform: "uppercase", color: "rgba(26,20,16,0.4)" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {GUIDE_TAILLES.map((row, i) => (
                    <tr key={row.taille} style={{ borderTop: "1px solid rgba(26,20,16,0.06)", background: i % 2 === 0 ? "#fff" : "#faf7f4" }}>
                      <td style={{ padding: "10px 12px", fontWeight: 900, color: "#c49a4a", fontSize: 13 }}>{row.taille}</td>
                      <td style={{ padding: "10px 12px", color: "rgba(26,20,16,0.6)", fontSize: 13 }}>{row.poids}</td>
                      <td style={{ padding: "10px 12px", color: "rgba(26,20,16,0.6)", fontSize: 13 }}>{row.hauteur}</td>
                      <td style={{ padding: "10px 12px", color: "rgba(26,20,16,0.6)", fontSize: 13 }}>{row.age}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <div style={{ padding: "8px 12px", fontSize: 11, color: "rgba(26,20,16,0.4)", background: "#f9f6f2", borderTop: "1px solid rgba(26,20,16,0.06)" }}>
          ...
          <div style={{ display: "grid", gap: 20 }}>
            ...
            <h1 style={{ margin: 0, fontSize: "clamp(24px, 2.5vw, 38px)", fontWeight: 950, letterSpacing: -1, lineHeight: 1.1, color: "#1a1410" }}>
              {product.name}
            </h1>

            <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
              <span style={{ fontSize: "clamp(28px, 3vw, 36px)", fontWeight: 950, letterSpacing: -1, color: "#1a1410" }}>{Number(displayPrice).toFixed(2)} €</span>
            ...
            {product.description && (
              <p style={{ margin: 0, fontSize: "clamp(14px, 1.4vw, 16px)", lineHeight: 1.8, color: "rgba(26,20,16,0.65)" }}>{product.description}</p>
            )}
              ...
              <div style={{ display: "grid", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  {couleursDispos.map((c: any) => {
                    ...
                    return (
                      <button key={c.name} onClick={() => { if (!epuise) setCouleur(c.name); }}
                        title={c.name}
            ...
            <div className="pl-dcta" style={{ gap: 10 }}>
              <button onClick={handleAddToCart} disabled={outTaille}
                style={{ padding: "18px 24px", borderRadius: 16, border: "none", fontWeight: 900, fontSize: "clamp(15px, 1.5vw, 17px)", cursor: outTaille ? "not-allowed" : "pointer", background: added ? "#2d6a2d" : outTaille ? "#d1cdc8" : "#1a1410", color: "#f2ede6", transition: "all 0.2s" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                ...
                <div key={r.label} style={{ padding: "10px 12px", borderRadius: 10, background: "#fff", display: "flex", alignItems: "center", gap: 7, fontSize: "clamp(11px, 1vw, 13px)", fontWeight: 700, color: "rgba(26,20,16,0.7)", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", whiteSpace: "nowrap" }}>
                  <r.Icon />
                  {r.label}
                </div>
              ))}
        ...
        <div className="pl-compo">
          <div style={{ padding: "24px 28px", borderRadius: 20, background: "#2a2018", color: "#f2ede6" }}>
            <h3 style={{ margin: "0 0 16px", fontSize: "clamp(15px, 1.5vw, 18px)", fontWeight: 950 }}>Composition & matière</h3>
            {[
              ...
              <div key={row.label} style={{ display: "flex", justifyContent: "space-between", gap: 12, paddingBottom: 8, borderBottom: "1px solid rgba(242,237,230,0.06)", marginBottom: 8 }}>
                ...
                <span style={{ fontSize: "clamp(12px, 1.1vw, 14px)", color: "rgba(242,237,230,0.7)", textAlign: "right" }}>{row.value}</span>
              </div>
            ))}
          </div>
          <div style={{ padding: "24px 28px", borderRadius: 20, background: "#fff", border: "1px solid rgba(26,20,16,0.07)" }}>
            <h3 style={{ margin: "0 0 16px", fontSize: "clamp(15px, 1.5vw, 18px)", fontWeight: 950, color: "#1a1410" }}>Entretien</h3>
            {[
              ...
              <div key={item.text} style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10 }}>
                ...
                <span style={{ fontSize: "clamp(12px, 1.1vw, 14px)", color: "rgba(26,20,16,0.7)", lineHeight: 1.4 }}>{item.text}</span>
              </div>
            ))}
          </div>
        </div>

        {/* ✅ FAQ compacte — espaces réduits, textes sur une ligne */}
        <div style={{ marginTop: 32, padding: "24px 28px", borderRadius: 20, background: "#2a2018" }}>
          <h3 style={{ margin: "0 0 4px", fontSize: "clamp(16px, 1.6vw, 20px)", fontWeight: 950, color: "#f2ede6" }}>Questions fréquentes</h3>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 56 }}>
            <h2 style={{ margin: "0 0 20px", fontSize: "clamp(18px, 2.5vw, 28px)", fontWeight: 950, letterSpacing: -1, color: "#1a1410" }}>Dans la même collection</h2>
