...
const BG       = "#ede8df";
const DARK     = "#1a1410";
// Durée totale de l'intro (ms) — ralentie ~+50% vs l'ancienne version,
// sans la phrase défilante.
const INTRO_DUR = 5600;
...
export default function IntroScreen() {
  ...
  useEffect(() => {
    ...
    // Timeline ralentie ~+50% (plus de phrase défilante) :
    //  50ms  → DOM prêt
    //  30ms  → M et LK apparaissent (phase 1)
    //  520ms → ! commence à tomber (phase 2, drop 1.1s)
    //  1650ms→ ! a atterri → néon démarre (phase 3, néon 3.3s)
    //  auto  → dismiss après le néon
    const dur = INTRO_DUR;
    const t = [
      ...
      {ready && (
        <div style={{
          ...
          <span style={{
            ...
            animation:
              phase >= 3 ? "mlk-neon 3.3s ease forwards" :
              phase >= 2 ? "mlk-drop 1.1s cubic-bezier(.22,.61,.36,1) forwards" :
              "none",
          }}>!</span>
          </span>
