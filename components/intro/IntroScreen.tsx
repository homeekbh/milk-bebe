...
const BG       = "#ede8df";
const DARK     = "#1a1410";
// Durée totale de l'intro (ms) — séquence compressée (~3.2s) pour une intro
// rapide, sans la phrase défilante.
...
export default function IntroScreen() {
  ...
  function dismiss() {
    ...
    // Tâche 2 — intro vue : on ne la rejouera plus pour ce visiteur.
    try { localStorage.setItem("milk_intro_seen", "1"); } catch {}
    setExiting(true);
    setTimeout(() => {
      setShow(false);
  ...
  useEffect(() => {
    ...
    // Tâche 2 — intro une seule fois par visiteur.
    // Le masquage VISUEL d'un revisiteur est fait AVANT le 1er paint par le
    // script inline bloquant du layout (#milk-intro{display:none}) → 0 flash.
    // Ici on relit localStorage au montage pour démonter proprement le
    // composant côté React. Aucun hydration mismatch : l'état initial `show`
    // ne dépend que de pathname (déterministe SSR/client), jamais de localStorage.
    let seen = false;
    try { seen = localStorage.getItem("milk_intro_seen") === "1"; } catch {}
    if (seen) {
      setShow(false);
      document.getElementById("intro-hide-hdr")?.remove();
      return;
