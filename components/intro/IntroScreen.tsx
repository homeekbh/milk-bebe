...
const BG       = "#f5f0e8";   // beige M!LK
const DARK     = "#1a1410";   // brun très foncé
const DARK2    = "#2d1a0e";   // variante légèrement plus claire
const PHRASE   = "PENSÉ POUR LES PARENTS... LE CONFORT QUI SIMPLIFIE VOS JOURNÉES.";

// Font Boldin Bold en base64 (OTF)
...
export default function IntroScreen() {
  ...
  // Effet machine à écrire pour la phrase
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Phase 1 : M L K apparaissent (fade sombre)
    const t1 = setTimeout(() => setPhase(1), 100);
    // Phase 2 : ! tombe et rebondit
    const t2 = setTimeout(() => setPhase(2), 450);
    // Phase 3 : effet néon + phrase se tape
    const t3 = setTimeout(() => setPhase(3), 1300);
    // Auto-dismiss
    const t4 = setTimeout(() => dismiss(), 1300 + PHRASE.length * 38 + 600);
    ...
    >
      {ready && (
        <style>{`
          ...
          @keyframes drop-bounce {
            0%   { transform: translateY(-280px) scaleY(0.7); opacity: 0; }
            10%  { opacity: 1; }
            50%  { transform: translateY(18px) scaleY(1.06); }
            62%  { transform: translateY(-12px) scaleY(0.96); }
            73%  { transform: translateY(7px) scaleY(1.02); }
            82%  { transform: translateY(-4px) scaleY(0.99); }
            90%  { transform: translateY(2px); }
            95%  { transform: translateY(-1px); }
            100% { transform: translateY(0) scaleY(1); opacity: 1; }
          }

          /* Néon électrique défaillant au moment du contact */
          @keyframes neon-flicker {
            0%   { color: ${DARK}; text-shadow: none; }
            /* atterrissage */
            5%   { color: #ffffff; text-shadow: 0 0 6px #fff, 0 0 20px #fff, 0 0 60px #fff; }
            12%  { color: ${DARK}; text-shadow: none; }
            /* premier problème électrique */
            15%  { color: #e8e8e8; text-shadow: 0 0 4px #fff, 0 0 15px rgba(255,255,255,0.7); }
            18%  { color: ${DARK}; text-shadow: none; }
            /* scintillement final */
            22%  { color: #ffffff; text-shadow: 0 0 8px #fff, 0 0 25px #fff, 0 0 80px rgba(255,255,255,0.5); }
            26%  { color: ${DARK}; text-shadow: none; }
            28%  { color: #d0d0d0; text-shadow: 0 0 3px #fff; }
            31%  { color: ${DARK}; text-shadow: none; }
            /* s'éteint définitivement */
            100% { color: ${DARK}; text-shadow: none; }
          }

          /* Sortie */
          @keyframes logo-exit {
            from { opacity: 1; transform: scale(1); }
            to   { opacity: 0; transform: scale(1.08) translateY(-8px); filter: blur(3px); }
          }
          ...
          .intro-bang-neon {
            animation: drop-bounce 0.72s cubic-bezier(0.22,0.61,0.36,1) forwards,
                       neon-flicker 1.8s ease 0.72s forwards;
          }
        `}</style>
      )}
      ...
      {ready && (
        <div
          style={{
            ...
            animation:      exiting ? "logo-exit 0.65s ease forwards" : undefined,
          }}
        >
          {/* M */}
          <span style={{
          ...
          {/* ! — tombe + néon électrique */}
          <span
            className={phase >= 2 ? "intro-bang-neon" : ""}
            style={{
              fontSize:    "clamp(82px, 17.5vw, 182px)",
              fontWeight:  900,
              color:       DARK,
              lineHeight:  1,
              display:     "inline-block",
      ...
      {ready && phase >= 3 && (
        <div style={{
          ...
          <span style={{
            ...
            animation:  typed.length < PHRASE.length ? "cursor-blink 0.7s step-end infinite" : "none",
            opacity:    typed.length < PHRASE.length ? 1 : 0,
          }} />
        </div>
      )}
      ...
      {ready && !exiting && (
        <div style={{
          ...
          background: DARK,
          opacity:    0.15,
          animation:  `progress-bar ${1300 + PHRASE.length * 38 + 300}ms linear forwards`,
        }} />
      )}
    </div>
  );
