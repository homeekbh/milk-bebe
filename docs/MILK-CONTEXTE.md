...
| Pack 100 % mono-taille : « achetable » même en rupture (quirk latent) | 17/08 | `lib/pack-availability.ts` (source unique fiche + flux Google Shopping) reproduit **à l'identique** la règle historique de la fiche : dans la branche `multi.length === 0` (aucun composant multi-taille), `monoOk` **n'est PAS appliqué** → `sizeRequired=false` → `isPackAvailable=true` **même si un mono est en rupture**. **Aucun pack réel concerné aujourd'hui** (tous ont ≥ 1 multi-taille). Conséquence si un pack 100 % mono apparaît : au **panier**, erreur (le pack ne part pas) ; dans le **flux**, article annoncé disponible → **refus Merchant Center**. Même défaut, deux enjeux. Reproduit volontairement — à corriger **avec décision explicite**, jamais « au jugé ». |
